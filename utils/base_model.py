...
from utils.thermoML import replace_string_with_nan
...
def create_model(trial):
    ...
    model.compile(optimizer=optimizer, loss="mae", metrics=["mse", "root_mean_squared_error"])
...
def main_base_train(df, df_mu, set_size, n_models = 20, hp_tuning = False):
    ...
    if hp_tuning = True:
        ...
        study.optimize(objective, n_trials=50)  # Number of trials to explore
    else:
        with open("data/base_model/Results/best_trial.pkl", "rb") as f:
    ...
    for j in range(n_models):
        ...
        history  = best_model.fit(X_train, y_train, validation_split=0.2, epochs=400, batch_size=32,
                                 callbacks = [tf.keras.callbacks.ModelCheckpoint(filepath =  f"data/base_model/Results/{j}/model.keras", save_best_only=True,verbose=1), 
...
def main_base_test(df_test, temp_test, feature_path):
   
    with open(os.path.join(feature_path, 'features.pkl'), 'rb') as file:
    ...
    y_pred_base_list = []
    for i in range(20):
        model = tf.keras.models.load_model("data/base_model/Results/2390/{}/model.keras".format(i), safe_mode=False)
    ...
    y_pred_base_certainty = y_pred_base_df_std < 5

    y_pred_base_avg_temp =  pd.DataFrame(y_pred_base_df_avg.values.reshape(5, df_test.shape[0]/5).T)
    y_pred_base_std_temp =  pd.DataFrame(y_pred_base_df_std.values.reshape(5, df_test.shape[0]/5).T)
    y_pred_base_certainty_temp =  pd.DataFrame(y_pred_base_certainty.values.reshape(5, df_test.shape[0]/5).T)
